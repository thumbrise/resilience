...
- **Multi-module isolation (target architecture)** — the goal is for every sub-package with external dependencies (otel, future circuit, grpc, etc.) to be a separate Go module with its own `go.mod`. Core stays zero-deps. Users pull only what they need. Currently, all packages live in a single root module (mono-module) — including `otel/`. Full multi-module support is blocked on [multimod](https://github.com/thumbrise/multimod) tooling. See devlog #3 for rationale.

...
func Parse(state multimod.State) (multimod.State, error) {
	...
	var root multimod.Module

	var subs []multimod.Module
...
func findAndParseModules(rootDir multimod.AbsDir) ([]multimod.Module, error) {
	var modules []multimod.Module
	...
	err := filepath.WalkDir(rootDir.String(), func(path string, d fs.DirEntry, err error) error {
		...
		if d.Name() != multimod.ModuleMarker {
			return nil
		}
...
func parseGoMod(gomodPath string) (multimod.Module, error) {
	data, err := os.ReadFile(gomodPath) //nolint:gosec // path from filepath.WalkDir
	if err != nil {
		return multimod.Module{}, fmt.Errorf("reading %s: %w", gomodPath, err)
	}

	file, err := modfile.Parse(gomodPath, data, nil)
	if err != nil {
		return multimod.Module{}, fmt.Errorf("parsing %s: %w", gomodPath, err)
	}

	if file.Module == nil {
		return multimod.Module{}, fmt.Errorf("parsing %s: %w", gomodPath, ErrNoModuleDirective)
	}
	...
	return multimod.Module{
		Path:      file.Module.Mod.Path,
		Dir:       multimod.AbsDir(filepath.Dir(gomodPath)), // safe: gomodPath is absolute from WalkDir
		GoVersion: goVersion,
		Requires:  requires,
		Replaces:  replaces,
	}, nil
